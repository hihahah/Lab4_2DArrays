...
// GUI components
   ...
   public void fillValues() {
       for (int row = 0; row < intArray.length; row++) {
           for (int col = 0; col < intArray[row].length; col++) {
               intArray[row][col] = 50 + (int) (Math.random() * 31); // Random number between 50 and 80
               animate(row, col); // Call animate for visualization
           }
       }
   }
   ...
   */
   public int findMinimum( int column )
   {
    ...
    // Write your code here:
	   int min = intArray[0][column]; // Start with the first value in the column
       for (int row = 1; row < intArray.length; row++) {
           if (intArray[row][column] < min) {
               min = intArray[row][column]; // Update min if a smaller value is found
           }
           animate(row, column, min); // Call animate for visualization
       }
    ...
    // Write your code here:
       ...
       for (int row = 0; row < intArray.length; row++) {
           for (int col = 0; col < intArray[row].length; col++) {
               ...
               animate(row, col, count); // Call animate for visualization
           }
       }
